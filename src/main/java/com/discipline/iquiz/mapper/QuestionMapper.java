...
public interface QuestionMapper {
    ...
    @Insert("INSERT INTO `question` (id,content,type,point,option_ids,answer_ids,qbank_id,tid,create_time,knowledge )" +
            "VALUES(#{id},#{content},#{type},#{point},#{optionIds},#{answerIds},#{qbankId},#{tid},#{time},#{knowledge})")
    int addQuestionToQbank(@Param("id")String id,@Param("content")String content,@Param("type")int type,
                              ...
                              @Param("tid")String tid,@Param("time")Date date,@Param("knowledge")String knowledge);
